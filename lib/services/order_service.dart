...
class OrderService {
  ...
  Future<List<OrderHistoryModel>> getHistoryByCustomer(String customerId) async {
    try {
      final response = await _apiService.get('/HoaDon/HistoryByKhachHang/$customerId');
      ...
      if (response.data['data'] != null) {
        ...
        if (data['history'] != null) {
          ...
          allOrders.addAll(historyList.map((e) => OrderHistoryModel.fromJson(e)));
        }

        // Parse current list
        if (data['current'] != null) {
          final List<dynamic> currentList = data['current'];
          allOrders.addAll(currentList.map((e) => OrderHistoryModel.fromJson(e)));
        }
      }
